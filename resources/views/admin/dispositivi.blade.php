...
<main class="main">
    <div class="page-header">
        <div><h1>📡 Dispositivi</h1><p>Tutti i dispenser PillMate nel sistema</p></div>
    ...
    <div class="card">
        <div class="table-wrap">
            <table>
                ...
                <tr>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#1e3a5f,#0e4d6e);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">💊</div>
                            <div>
                    ...
                    <td style="font-size:13px;font-weight:{{ $d->temperatura > 35 ? '700' : '400' }};color:{{ ($d->temperatura ?? 0) > 35 ? 'var(--red)' : 'var(--text)' }};">
                        {{ $d->temperatura !== null ? $d->temperatura.'°C' : '—' }}
                    </td>
                    <td style="font-size:13px;">{{ $d->umidita !== null ? $d->umidita.'%' : '—' }}</td>
                    ...
                    <td style="font-size:13px;">{{ $d->scomparto_attuale !== null ? 'N° '.$d->scomparto_attuale : '—' }}</td>
                    <td>
                        @if($d->allarme_attivo)
                            <span style="background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700;">🔔 Sì</span>
                        @else
                ...
                @empty
                <tr><td colspan="9"><div class="empty-state">Nessun dispositivo registrato nel sistema.</div></td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
