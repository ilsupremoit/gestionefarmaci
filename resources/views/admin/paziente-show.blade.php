...
<head>
    ...
    <meta charset="UTF-8"/>
    <title>PillMate Admin — Paziente</title>
...
<main class="main">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;flex-wrap:wrap;">
        <a href="{{ route('admin.pazienti') }}" class="btn btn-ghost btn-sm">
        ...
        <div class="card">
            ...
            <div style="display:grid;gap:10px;font-size:13px;">
                ...
                @if($paziente->note_mediche)
                    <div><span style="color:var(--muted);">Note:</span> {{ $paziente->note_mediche }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="card">
            ...
            @forelse($paziente->dispositivi as $d)
                <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
                    <div style="width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#ede9fe,#dbeafe);display:flex;align-items:center;justify-content:center;color:var(--accent);flex-shrink:0;">
                        <i data-lucide="pill"></i>
                    </div>

                    <div style="flex:1;">
                        <div style="font-weight:600;font-size:13px;">{{ $d->nome_dispositivo ?? $d->codice_seriale }}</div>
                        <div style="font-size:11px;color:var(--muted);">{{ $d->codice_seriale }}</div>
                    </div>
        ...
        @forelse($paziente->terapie->where('attiva',true) as $t)
            <div style="padding:10px;border:1px solid var(--border);border-radius:10px;margin-bottom:8px;">
                ...
                <div style="font-size:12px;color:var(--muted);">
                    Dal {{ $t->data_inizio->format('d/m/Y') }} · {{ $t->quantita }} pillola/e
                </div>
            </div>
        @empty
            <div class="empty-state">Nessuna terapia attiva.</div>
        @endforelse
    </div>
    ...
    <div class="card">
        ...
        @forelse($assunzioni as $a)
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
                ...
                <span class="ruolo-badge" style="background:{{ in_array($a->stato,['assunta','erogata']) ? '#f0fdf4' : (in_array($a->stato,['saltata','non_ritirata']) ? '#fef2f2' : '#fff7ed') }};color:{{ in_array($a->stato,['assunta','erogata']) ? '#15803d' : (in_array($a->stato,['saltata','non_ritirata']) ? '#b91c1c' : '#c2410c') }}">
                    {{ ucfirst(str_replace('_',' ',$a->stato)) }}
                </span>
            </div>
        @empty
            <div class="empty-state">Nessuna assunzione registrata.</div>
        @endforelse
    </div>
</main>
</body>
