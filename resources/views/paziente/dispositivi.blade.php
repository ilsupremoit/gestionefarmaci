...
<main class="main">
    ...
    <div class="card"><div class="empty-state" style="padding:48px 0;">
        📡<br><br>Nessun dispositivo associato.<br>
    ...
    <div class="device-grid">
        ...
        <div class="device-card">
            ...
            <div>
                @if($d->allarme_attivo)
                    <span class="device-status" style="background:rgba(239,68,68,.2);color:#fca5a5;border:1px solid rgba(239,68,68,.4);animation:pulse 1.5s infinite;">
                ...
                @elseif($online)
                    <span class="device-status" style="background:rgba(245,158,11,.12);color:#fcd34d;border:1px solid rgba(245,158,11,.2)"><span class="dot dot-yellow"></span> Connesso (inattivo)</span>
            ...
            <div class="device-metrics" style="margin-top:14px;">
                ...
                <div class="metric">
                    ...
                    <div class="metric-value" style="color:{{ $d->temperatura > 35 ? 'var(--red)' : 'var(--text)' }}">{{ $d->temperatura }}°C</div>
                </div>
                @endif
                @if($d->umidita !== null)
                <div class="metric">
                ...
                <div class="metric">
                    ...
                    <div class="metric-value" style="color:{{ $d->batteria < 20 ? 'var(--red)' : ($d->batteria < 50 ? 'var(--yellow)' : 'var(--green)') }}">{{ $d->batteria }}%</div>
                </div>
                @endif
                @if($d->wifi_rssi !== null)
                <div class="metric">
                    <div class="metric-label">📶 Segnale WiFi</div>
                    <div class="metric-value" style="color:{{ $d->wifi_rssi < -80 ? 'var(--red)' : ($d->wifi_rssi < -60 ? 'var(--yellow)' : 'var(--green)') }}">{{ $d->wifi_rssi }} dBm</div>
                </div>
                @endif
            ...
            {{-- Configurazione broker (info read-only per familiare) --}}
            <div style="margin-top:12px;padding:10px 12px;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:8px;">
                <div style="font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);font-weight:600;margin-bottom:6px;">📡 Configurazione MQTT</div>
    ...
    <div class="card" style="margin-top:24px;">
        <div class="card-title">ℹ️ Legenda stati dispositivo</div>
