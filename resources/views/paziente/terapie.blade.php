...
<main class="main">
        ...
        <div class="terapia-card">
            <div class="terapia-card-header">
                <div>
                    ...
                    @if($farm?->dose)<div class="terapia-card-dose">{{ $farm->dose }}@if($farm->descrizione) · {{ Str::limit($farm->descrizione,60) }}@endif</div>@endif
                </div>
                <span class="badge-attiva">✅ Attiva</span>
            </div>

            <div class="terapia-meta">
                <span>📅 Dal {{ $t->data_inizio->format('d/m/Y') }}@if($t->data_fine) al {{ $t->data_fine->format('d/m/Y') }}@endif</span>
                <span>💊 {{ $t->quantita }} pillola/e per dose</span>
                @if($t->frequenza)<span>⏱ {{ $t->frequenza }}</span>@endif
                @if($t->medico)<span>👨‍⚕️ Dr. {{ $t->medico->cognome }}</span>@endif
            </div>
            ...
            @if($t->istruzioni)
            <div class="istruzioni-box">📝 <strong>Istruzioni:</strong> {{ $t->istruzioni }}</div>
            @endif
            ...
            <div style="margin-top:14px;">
                <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--muted);margin-bottom:4px;">
                    ...
                    <span style="font-weight:700;color:{{ $aderenza >= 80 ? '#6ee7b7' : ($aderenza >= 50 ? '#fcd34d' : '#fca5a5') }}">{{ $aderenza }}%</span>
                </div>
            ...
            @if($t->medico)
            <div class="medico-info">👨‍⚕️ Prescritta dal Dr. {{ $t->medico->cognome }} {{ $t->medico->nome }}
                @if($t->medico->telefono) · 📞 {{ $t->medico->telefono }}@endif
            </div>
            @endif
        </div>
        @empty
        <div class="card"><div class="empty-state">Nessuna terapia attiva al momento. Il medico non ha ancora prescritto terapie.</div></div>
        @endforelse
    </div>
    ...
    <div>
        ...
        <div class="terapia-card" style="opacity:.65">
            ...
            <div class="terapia-meta">
                <span>📅 Dal {{ $t->data_inizio->format('d/m/Y') }}@if($t->data_fine) al {{ $t->data_fine->format('d/m/Y') }}@endif</span>
                <span>💊 {{ $t->quantita }} pillola/e</span>
                @if($t->medico)<span>👨‍⚕️ Dr. {{ $t->medico->cognome }}</span>@endif
            </div>
            @if($t->istruzioni)<div class="istruzioni-box">📝 {{ $t->istruzioni }}</div>@endif
        </div>
        @endforeach
    </div>
    @endif
